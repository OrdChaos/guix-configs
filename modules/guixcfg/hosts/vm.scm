...
(define %vm-services
  (append
   (list ;; QEMU user-mode 网络（SLIRP：DHCP 10.0.2.15 / DNS 10.0.2.3）：
    ;; NetworkManager——Guix 官方 DNS 管理主路径（%desktop-services
    ;; 同款；NM 包 patch-resolvconf-path 集成 openresolv）。不用
    ;; dhcpcd：dhcpcd hook 走 openresolv 3.17.4 时会被 /etc/resolv.conf
    ;; 的 Guix 占位普通文件（无 "# Generated by resolvconf" ownership
    ;; 标记）挡住，DNS 永不写入（2026-08 实测；Guix News: openresolv
    ;; update requires restarting NetworkManager）。
    ;;
    ;; 注意 NM 自己同样走 openresolv（pinned Guix 以
    ;; -Dconfig_dns_rc_manager_default=resolvconf 编译）：Guix
    ;; nscd-activation 的 placeholder 会以同样方式挡住 NM 每 boot 的
    ;; resolvconf -a（signature mismatch——2026-08 实测，每 boot 需
    ;; 手工 rm /etc/resolv.conf + 重启 NM）。resolvconf-bootstrap
    ;;（(guixcfg system resolvconf)）在 NM 之前用官方 -u 接管
    ;; placeholder / 缺失文件——NM 显式依赖它（见下）。
    (service network-manager-service-type
             ;; VM 无 WiFi：保留默认 '(wireless-daemon) requirement
             ;; 会因缺服务 fail-fast（pinned network-manager-configuration
             ;; 默认 '(wireless-daemon)）——因此不保留该默认；基础
             ;; requirement user-processes/dbus-system/loopback 由
             ;; pinned network-manager-shepherd-service 自带，不受本
             ;; 字段影响（tests/test-resolvconf-bootstrap.scm RB2 断言）。
             (network-manager-configuration
              (shepherd-requirement '(resolvconf-bootstrap))))
    ;; DNS boot init：activation 之后、NetworkManager 之前，用
    ;; openresolv 官方 -u 使 /etc/resolv.conf 进入 openresolv 合法
    ;; 所有状态（只接管 Guix nscd placeholder 或缺失文件；不持久化、
    ;; 不构造 nameserver——见模块头）。
