    ...
    (append
     (list
      ...
      (plan-step (id 'confirm-target)
                 (summary "Validate target device and require confirmation")
                 (detail `((device . ,device))))
      (plan-step (id 'wipe)
                 (summary "Wipe existing partition table and signatures from disk")
                 (detail `((device . ,device))))
      ;; 一条 sgdisk 命令完成：GPT + ESP + 系统分区 + 类型码 + PARTLABEL。
      (plan-step (id 'partition)
                 (summary "Create GPT, ESP partition and encrypted system partition")
                 (detail `((device . ,device)
                           (esp-size . ,esp-size))))
      ...
      (plan-step (id 'mount-top)
                 (summary "Temporarily mount Btrfs top level to create subvolumes")
                 (detail `((device . ,mapper-path)))))
     
     ;; 8 个固定持久子卷（顺序来自 model.scm）。
     (map (lambda (sv)
            (plan-step (id 'make-subvolume)
                       (summary (string-append "Create persistent subvolume " (subvolume-name sv)))
     ...
     (list
      ...
      (plan-step (id 'make-swapfile)
                 (summary "Create Btrfs swapfile (NOCOW, no compression, preallocated)")
     ...
     (map (lambda (sv)
            (plan-step (id 'mount-subvolume)
                       (summary (string-append "Mount " (subvolume-name sv)
                                               " at /mnt" (subvolume-mount-point sv)))
                       (detail `((name . ,(subvolume-name sv))
     ...
     (list
      ...
      (plan-step (id 'write-facts)
                 (summary "Write machine facts file /persist/system/facts/host.scm")
                 (detail `((target . "/mnt"))))
      (plan-step (id 'ready)
