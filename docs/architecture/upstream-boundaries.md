...
|---|---|---|---|---|
| Niri user lifecycle | guixcfg thin adapter | Guix Home `home-niri-service-type` | `home-niri-session-service-type`（no respawn + logout/session termination） | 项目需要严格的会话退出语义 |
...
| Fallback tty | Guix official | mingetty（tty2-6，gated by interactive-session-ready） | `hosts/common.scm`（tty1 移除 + gating） | 官方服务 + 本地 gating |
...
- niri 由 **home-niri-session-service-type**（官方服务的本地 thin
  adapter，Home Shepherd）启动。
