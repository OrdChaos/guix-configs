...
gate transaction → drift check），但不执行 enrollment。成功后必须先
重启到新 UKI，再显式运行 `blue enroll HOST`。该入口只允许在固件 PK
写入前执行；PK 写入后 lifecycle guard 会在 reconfigure 之前阻断，后续
更新使用普通 `blue reconfigure HOST`。
