...
vm-ssh() {
    # sshd 依赖网络就绪（dhcpcd）与首次 host key 生成，root@ordchaos-vm
...
sb-enroll() {
    T7_KEEP_VM=1 tests/integration/t3/boot.sh interact sb-enroll \
        "wait:Enter passphrase|send:$RECOVERY_PW|wait:root@ordchaos-vm" >/dev/null
...
scenario() {
    ...
    case "$name" in
        ...
        D|d)
            ...
            sleep 8
            T7_KEEP_VM=1 tests/integration/t3/boot.sh interact stage-b \
                "wait:Enter passphrase|send:$RECOVERY_PW|wait:root@ordchaos-vm" \
