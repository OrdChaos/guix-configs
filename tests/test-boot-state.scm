...
(let* ((dir (mkdtemp "/tmp/guixcfg-boot-state-XXXXXX"))
  ...
  (dynamic-wind
   ...
   (lambda ()
     ...
     (test-equal "reads last-good generation" 7 (read-boot-states path))
     ...
     (test-equal "falls back to .prev on corrupt structure" 7 (read-boot-states path))
     ...
     (test-equal "v2 reads generation" 9 (read-boot-states path))
     ...
     (let ((v1 (string-append dir "/v1.scm")))
       (call-with-output-file v1
                              (lambda (port)
                                ...
                                (newline port)))
       (test-equal "v1 compat generation" 3 (read-boot-states v1))
       (test-equal "v1 compat cmdline" "root=/x" (read-boot-command-line v1))
       (test-equal "v1 last-good system is #f"
